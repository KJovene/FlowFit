...
function MySessionsPage() {
  ...
  useEffect(() => {
    ...
    loadSessions();
  }, [isAuthenticated, filterShared, sortOrder]);
  ...
  const sharedCount = sessions.filter((s) => s.isShared).length;
  const privateCount = sessions.filter((s) => !s.isShared).length;
    ...
    <section className="mx-auto max-w-6xl px-4 sm:px-6 pt-10 sm:pt-16 lg:pt-20 pb-10 sm:pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-base text-neutral-300">
            Gérez vos séances personnelles et partagées
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-neutral-400">
          {filterShared === "all" && "Vous n'avez pas encore créé de séance"}
          {filterShared === "shared" && "Aucune séance partagée"}
          {filterShared === "private" && "Aucune séance privée"}
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredByCategory.map((session) => {
